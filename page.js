...
function showResults() {
    ...
    if (record && record.runs > 1) {
        ...
        const { container: bestContainer, canvas: bestCanvas } = createChartContainer('Best Run');
        bestColumn.appendChild(bestContainer);

        const worstColumn = document.createElement('div');
        worstColumn.className = 'chart-column';
        const { container: worstContainer, canvas: worstCanvas } = createChartContainer('Worst Run');
...
function tryHitLane(lane) {
    ...
    addFloatText(x, hitY - 10, `${judgement.label} (${msText}ms)`, judgement.color);
}
